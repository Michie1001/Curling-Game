...
void ball::Reset(void)
{
	...
	static const float sep = (BALL_RADIUS*3.5f);
	static const float rowSep = (BALL_RADIUS*5.0f);
	int row = 1;
	int rowIndex = index;
	while(rowIndex > row)
	{
...
void table::SetupCushions(void)
{
	...
	cushions[0].vertices[0](1) = -TABLE_Z - 6;
	...
	cushions[2].vertices[1](1) = -TABLE_Z - 6;

	//top border
	cushions[3].vertices[0](0) = TABLE_X;
	cushions[3].vertices[0](1) = -TABLE_Z - 6;
	cushions[3].vertices[1](0) = TABLE_X - 1.2;
	cushions[3].vertices[1](1) = -TABLE_Z - 6;
